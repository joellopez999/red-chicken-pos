...
| **Overbooking slots** | Count of reservation time slots in the selected date range where party/guest demand exceeds venue capacity (`reservations.overbooking_slots_count`). Shown as a summary card only when the count is **greater than 0**. Design and prevention behaviour: [0025-reservation-overbooking-detection.md](0025-reservation-overbooking-detection.md). |
...
- **GET `/reports/sales?from_date=YYYY-MM-DD&to_date=YYYY-MM-DD`**  
  Returns combined report: `summary` (includes `average_revenue_per_order_cents`), `reservations` (total, by_source, **`overbooking_slots_count`**), `by_product`, `by_category`, `by_table`, `by_waiter`. Requires auth and `report:read`.
