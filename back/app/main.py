...
from .sri_signing import EC_TZ, load_p12
...
app.include_router(ai_phone_menu_router, tags=["AI Phone Menu"])
...
def update_order_status(
    ...
    # A paid order is already a terminal state (kitchen/waiter work is done AND it's collected).
    # A later delivery-tracking update — e.g. clicking "Completar pedido" to close out a
    # pre-paid order once all items are out — must not downgrade it back to "completed" and
    # lose the "Pagado" label; item statuses below still update normally.
    if order.status != models.OrderStatus.paid or status_update.status == models.OrderStatus.cancelled:
